...
fun ForecastResponseDto.toForecast(): Forecast {
    ...
    val currentWeather = Weather.Current(
        time = LocalDateTime.parse(current.time, DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm")),
    ...
    for(i in 0 until hourly.time.size) {
        hourlyWeather.add(
            Weather.Hourly(
                time = LocalDateTime.parse(hourly.time[i], DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm")),
    ...
    for(i in 0 until daily.time.size) {
        dailyWeather.add(
            Weather.Daily(
                time = LocalDateTime.parse(daily.time[i] + "T00:00", DateTimeFormatter.ofPattern("yyyy-MM-dd'T'HH:mm")),
